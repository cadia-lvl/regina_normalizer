# ORDINAL
from number_patterns import hndrds_ptrn_no11_def, ones_ptrn_no11, tns_ptrn, hndrds_ptrn_11, ones_ptrn_11
...
for rule, letter in dozens_ordinal_letters:
    ordinal_thousands_tuples.append((hndrds_ptrn_no11_def + "1([01][1-9]|10)\.$", rule, 'hundreds', ' eitt hundrað' + letter + ' og'))
    ordinal_thousands_tuples.append(("^1([01][1-9]|10)\.$", rule, 'hundreds', 'hundrað' + letter + ' og'))
    ordinal_thousands_tuples.append((hndrds_ptrn_no11_def + "100\.$", rule, 'hundreds', ' eitt hundrað' + letter))
    ...
    for string, number in hundreds_thousands_zip:
        ordinal_thousands_tuples.append((hndrds_ptrn_11 + number + "([01][1-9]|10)\.$", rule, 'hundreds', string + ' hundruð' + letter + ' og'))
        ordinal_thousands_tuples.append((hndrds_ptrn_11 + number + "00\.$", rule, 'hundreds', string + ' hundruð' + letter))
